...
/**
 * Coerce a Claude response that should be JSON into a parsed object.
 * Tolerates ```json fences and stray prose around the JSON.
 */
export function parseClaudeJson<T = unknown>(raw: string): T {
  ...
  // Otherwise pull the first {...} or [...] block.
  if (!s.startsWith("{") && !s.startsWith("[")) {
    const objMatch = s.match(/\{[\s\S]*\}/);
    const arrMatch = s.match(/\[[\s\S]*\]/);
    s = (objMatch?.[0] ?? arrMatch?.[0] ?? s).trim();
  }
