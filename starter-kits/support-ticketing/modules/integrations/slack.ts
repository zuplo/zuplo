 ...
 * for DMs and to support threading).
 *
 ...
 * available — required if `channel` is a user id (DM) or if you want a
 * `thread_ts` in the response. Falls back to the incoming webhook URL.
 */
export async function postSlackMessage(
  ...
  if (botToken) {
    const channel =
      msg.channel ?? environment.SLACK_DEFAULT_CHANNEL ?? "#general";
...
/**
 * Look up a user id by email, then DM them. Useful for assignment notifications.
 */
