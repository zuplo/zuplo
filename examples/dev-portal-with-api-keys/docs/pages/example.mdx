...
It makes a signed request to the proxy API on the gateway exposed by `api-key.oas.json` with the nececssary body content such as `email`, `userId` and `name` if they are available.
...
In this example, that policy is set up to work with Auth0, which is also the default demo authentication provider. To use this with your own auth provider, you will need to change the configuration for these.
